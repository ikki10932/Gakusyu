...
/*******************************************************************************
  ...
  ※このクラスは　IPSEC対応ルータ(RTX1000系)の読み書き用のクラスである
...
CDkRoutePassEx m_cRtPassEx1000;
...
BOOL CDkRouter1000::_Put(char *buf, BOOL fKaigyo/*=TRUE*/)
{
	CString strBuf;
	strBuf = buf;
//===== ↓LTE通信対応 ==================================================================
	// ケーブル光でpp [ADSL番号]が来た時はpp Gateway [ゲートウェイアドレス]に変える
	CString strAdslPp, strAdslGateway;
	SDkConnectInfoNew *pInfo = GetAdslInfo();
	
	CString strFind1, strFind2;
	
	if ( pInfo && m_sRouter.port[3].kaisen == eKaisenLTE )
	{
		while ( TRUE )
		{
			strFind1.Format("stealth pp %d", pInfo->no);
			if ( strBuf.Find(strFind1) > -1 )
			{
				strFind2.Format("stealth wan1");
				strBuf.Replace(strFind1, strFind2);
				break;
			}

			strFind1.Format("pp %d", pInfo->no);
			if ( strBuf.Find(strFind1) > -1 )
			{
				strFind2.Format("dhcp wan1");
				strBuf.Replace(strFind1, strFind2);
				break;
			}
/*			
			if ( strBuf.Find("ipsec ike pre-shared-key ") > -1 )
			{
				strFind1.Format("%s", GetIpB2C(pInfo->fixIp));
				strFind2.Format("%s", GetIpB2C(m_sRouter.sLte.ip));
				strBuf.Replace(strFind1, strFind2);
				break;
			}
			
			if ( strBuf.Find("ipsec ike local address ") > -1 )
			{
				strFind1.Format("%s", GetIpB2C(pInfo->fixIp));
				strFind2.Format("%s", GetIpB2C(m_sRouter.sLte.ip));
				strBuf.Replace(strFind1, strFind2);
...
*/			
			if ( strBuf.Find("nat descriptor address outer 2000 ") > -1 )
			{
// 				strFind1.Format("%s", GetIpB2C(pInfo->fixIp));
// 				strFind2.Format("primary");
// 				strBuf.Replace(strFind1, strFind2);
				strBuf = "nat descriptor address outer 2000 primary";
				break;
			}
			
			if ( strBuf.Find("dns server") > -1 && strBuf.Find("dns server pp") < 0 )
			{
				strBuf = "dns server 8.8.8.8 8.8.4.4";
				break;
			}
			break;
		}
...
//===== ↑LTE通信対応 ==================================================================
	else
	{
		// ケーブル光でpp [ADSL番号]が来た時はpp Gateway [ゲートウェイアドレス]に変える
		SDkConnectInfoNew *pInfo = GetAdslInfo();
		if ( pInfo && m_sRouter.port[1].kaisen == eKaisenCableHikari )
		{
			//===== ↓DHCP対応 ====================================================================
			...
			{	
				...
				while ( TRUE )
				{
					...
					if ( strBuf.Find(strFind1) > -1 )
					{
						strFind2.Format("stealth lan2");
						nRep = strBuf.Replace(strFind1, strFind2);
						break;
					}
					strFind1.Format("pp %d", pInfo->no);
					if ( strBuf.Find(strFind1) > -1 )
					{
						strFind2.Format("dhcp lan2");
						nRep = strBuf.Replace(strFind1, strFind2);
						break;
					}
					if ( strBuf.Find("nat descriptor address outer 2000 ") > -1 )
...
// 					if ( strBuf.Find("dns server pp") < 0 )
// 					{
// 						strBuf = "dns server dhcp lan2";
// 						break;
// 					}
					break;
				}
				if (nRep > 0)
					strBuf.Replace(" metric 2", "");	// metric 2がついてたら消す
			}
			//===== ↑DHCP対応 ====================================================================
			else
			{
				CString strAdslPp;
				CString strAdslGateway;
				strAdslPp.Format("pp %d", pInfo->no);
				strAdslGateway.Format("%s", GetIpB2C(pInfo->yobi2));
				
				if ( strBuf.Find(strAdslPp) > -1 )
				{
					if (pInfo)
					{
						int nRep = strBuf.Replace(strAdslPp, strAdslGateway);
						if (nRep > 0)
							strBuf.Replace(" metric 2", "");	// metric 2がついてたら消す
					} else {
						return FALSE;
	...
	return CDkRouter::_Put((CStoS)strBuf, fKaigyo);
}

BOOL CDkRouter1000::_Put(const char *buf[], int n, BOOL fKaigyo/*=TRUE*/)
{
	return CDkRouter::_Put(buf, n, fKaigyo);
}
BOOL CDkRouter1000::_PutTit(char *buf)
{
	return CDkRouter::_PutTit(buf);
}
BOOL CDkRouter1000::_Put(CString strCmd)
{
	return CDkRouter::_Put(strCmd);
}
...
//		pLogにはログファイル出力クラス
CDkRouter1000::CDkRouter1000(SDkMcnInfo *pDkLan, CFile *pLog/*=NULL*/)
{
	m_nArray		= 0;
...
//	<戻値>	TRUE = 成功    FALSE = 失敗
//----------------------------------------------------------------------------
BOOL CDkRouter1000::PreReadFile(char* szFile)
{
...
// 			if ( p && strncmp(p, "192.168.210.3", lstrlen(p)) == 0 )	// JNETメインのＩＰ
// 			{
// 				p = strtok(NULL, SEARCH_CHR);	// gateway
// 				p = strtok(NULL, SEARCH_CHR);	// tunnel or pp
// 				p = strtok(NULL, SEARCH_CHR);
// 				if ( p )
// 				{
// 					m_sRouter.m_nJnetNo = atoi(p);
// 				}
// 			}
// 			if ( p && strncmp(p, "192.168.210.3", lstrlen(p)) == 0 )		// JNETサブのＩＰ
// 			{
// 				p = strtok(NULL, SEARCH_CHR);	// gateway
// 				p = strtok(NULL, SEARCH_CHR);	// tunnel or pp
// 				p = strtok(NULL, SEARCH_CHR);
// 				if ( p )
// 				{
// 					m_sRouter.m_nJnetNo2 = atoi(p);
// 				}
// 			}
		}
// 		if ( strAnalize.Find("esp 3des-cbc sha-hmac") != -1 )	// もうこの手はつかえない
// 		{
// 			nSIS05L2 = nSetuzokuNum;
// 		}
...
//	<戻値>	TRUE = 成功    FALSE = 失敗
//----------------------------------------------------------------------------
BOOL CDkRouter1000::ReadTftpFile(int no, char* szFile)
{
//===== ↓クラウドリニューアル対応 ========================================================
...
//===== ↑ケーブル光対応 ==================================================================
	...
	if ( szFile == NULL )	// ファイル名を指定して読み込まないとき
		szFile = GetFileName(no, eRead);	// ルータ番号にしたがってファイル名を作成
...
// 		char szLine[256];
// 		lstrcpy(szLine, szBuf);	// 判断を簡略化させるために追加 07/08/31
		m_strReadLine = szBuf;
...
//	<戻値>	TRUE = 成功    FALSE = 失敗
//----------------------------------------------------------------------------
int CDkRouter1000::AnalizeRouterType(char *buf)
{
	char	*p;
	BOOL	fFind = FALSE;
	...
	for ( int type = 0; type < DK_Number(szRtType); type++ )
	{
		if (p && strncmp(p, szRtType[type], lstrlen(szRtType[type])) == 0)
		{
			...
			case eRT103i:	case eRT52pro:	case eRTA52i:
				...
				m_sRouter.port[3].kaisen = eKaisenNoEdit;	// 使用不可
			...
			case eRTA54i:	case eRTA55i:	case eRT57i:	case eRT58i:
				...
				break;
			case eRTX1000: case eRTX1000_OA: case eRTX1100: case eRTX1100_OA: case eRTX1200: case eRTX1200_OA: case eRTX1210: case eRTX1210_OA: case eRTX1220: case eRTX1220_OA:
				m_sRouter.port[0].kaisen = eKaisenBlank;	// とりあえずブランク
				m_sRouter.port[1].kaisen = eKaisenBlank;	// とりあえずブランク
				...
				if ( m_thisIp[0] == 192 && m_thisIp[1] == 168 && (m_thisIp[2] == 60 || m_thisIp[2] == 61) && m_thisIp[3] == 1 ) 
				{
					...
					else if ( m_sRouter.kind == eRTX1200 )
...
//-----------------------------------------------------------------------------
int CDkRouter1000::Analize(char *buf)
{
...
// PPPOEの処理をする
int CDkRouter1000::ProcPPPOE(char *buf)
{
...
// DNSの処理をする
int CDkRouter1000::ProcDNS(char *buf)
{
	char *p = strtok(NULL, SEARCH_CHR);
...
// analogの処理をする
int CDkRouter1000::ProcAnalog(char *buf)
{
	const char *use = "use";
	const char *off = "off";
	char *p;
...
//	PPの処理をする
int CDkRouter1000::ProcPP(char *buf)
{
	const char *szCmd01[] = {
...
//	IPの処理をする
int CDkRouter1000::ProcIP(char *buf)
{
...
//===== ↓ケーブル光対応 ==================================================================
			if (p && strncmp(address, p, lstrlen(address)) == 0 )
			{
				...
				else
				{
					...
					{
						m_sRouter.fUseLan2DHCP = TRUE;	// ip lan2 address dhcp
					}
					//===== ↑DHCP対応 ====================================================================
					else
					{
...
//===== ↑J-NET顧客代行対応 ==========================================================
			...
			if ( p && strncmp("default", p, lstrlen("default")) == 0 )  // default
			{
//===== ↓ケーブル光対応 ==================================================================
				p = strtok(NULL, SEARCH_CHR);
...
//===== ↓ケーブル光対応 ==================================================================
					if ( !m_fBBsetLAN3 )
					{
						m_sRouter.fBbUse = eLaboBB_FixAdsl;	// ケーブル光として選択される
					}
//===== ↑ケーブル光対応 ==================================================================
					else
					{
...
//	NATの処理をする
int CDkRouter1000::ProcNAT(char *buf)
{
...
BOOL CDkRouter1000::ProcTunnel(char *buf)
{
	char *p = strtok(NULL, SEARCH_CHR);
...
BOOL CDkRouter1000::ProcDHCP(char *buf)
{
	char	*p = strtok(NULL, SEARCH_CHR);
...
BOOL CDkRouter1000::ProcIPSEC(char *buf)
{
...
//	<引数>	int		no		= ルータNO
//          char	*szFile = ファイル名
//			BOOL	fStatic	= 静的ルートセットをするか？
//	<戻値>	int 1:成功		0:登録エラー	-1:静的ルートセットエラー
//-----------------------------------------------------------------------------
int CDkRouter1000::WriteTftpFile(int no, char *szFile/*=NULL*/, BOOL fStatic/*=FALSE*/, char *szPH_CodeA2910, BYTE *pSendSrcIp/*=NULL*/)
{
	// ※ルータ種別がISDNの場合は継承元を使う
	if ( m_sRouter.kind == eRT103i || m_sRouter.kind == eRT52pro || m_sRouter.kind == eRTA52i )
	{
	// 	return CDkRouter::WriteTftpFile(no, szFile, fStatic);
		return 0;
	}
	if ( m_sRouter.kind == eRTA54i || m_sRouter.kind == eRTA55i || RT57or58 )
	{
	// 	return CDkRouterBB::WriteTftpFile(no, szFile, fStatic);
		return 0;
	}
	
	// A29XX特別対応版の場合はiniにPH会員番号を書いて(CDkRouter1200で書いてる)再読み込み2019/08/21
#if A2910_PH_INP_SHOW
	if ( GetReleaseVersion() < A3000 || !IsXai() )
	{
		m_cRtPassEx1000.m_nPH_Code = m_cRtPassEx1000.GetPHcode();
	}
#endif
	...
	CString sVPN_UserKey =		  CDkRoutePass::createKeyByMac(sMac, m_cRtPassEx1000.m_nPH_Code);			// ② VPNユーザKEY->名
	lstrcpy(m_sRouter.szVPN_User, CDkRoutePass::getVpnParam(sVPN_UserKey, eRtUser));
	lstrcpy(m_sRouter.szVPN_Pass, CDkRoutePass::getVpnKey(m_cRtPassEx1000.m_nPH_Code, szGlobalIp, 1, 0));	// ③ VPNパスワード
	CString sVPN_Key =			  CDkRoutePass::getVpnKey(m_cRtPassEx1000.m_nPH_Code, szGlobalIp, 2, 0);	// ④ VPN PRE-SHARED-KEY
	lstrcpy(m_sRouter.szVPN_Key , CDkRoutePass::getVpnParam(sVPN_Key, eRtPreSk));
	
	// ⑤ LUA待受ポート, ⑥ フリーPC待受ポート, ⑧ RDP待受ポート	
	CDkRoutePass::calcPort(m_cRtPassEx1000.m_nPH_Code, &nLuaPort, &m_sRouter.nFreePcPort, &m_sRouter.nRdpRecvPort);
	m_sRouter.nRdpOpenPort = 58301;	 	// ⑦ RDP開放ポート	 固定
	
// 	m_nRegRdpPort = m_sRouter.nRdpRecvPort;	// レジストリ書き込み用(ＲＤＰのポート番号のレジストリに書き込みするのはXaiPCのRTX1200系のみ。RTX1000系を使っている本部には必要ない)
	//===== ↑VPN情報取得処理 ==============================================================
...
//	Config 書き出し
BOOL CDkRouter1000::PutConfig(char *tit)
{
	char	szBuf[256];
	...
	if (!_Put("no schedule at 1"))	// scheduleコマンドはclear configurationで消去不可なのでこのコマンドで消す
		return FALSE;
...
//	ISDN情報書き出し
BOOL CDkRouter1000::PutIsdn(char *tit)
{
	char	szBuf[256];
...
//	<戻値>	TRUE = 成功    FALSE = 失敗
//=----------------------------------------------------------------------------------------------------
BOOL CDkRouter1000::PutFilterPass(int nFilterOrg, int nPass, BOOL fRtIpBefor, BOOL fNetAddress/*=FALSE*/)
{
	CString strPut;
...
//	<戻値>	TRUE = 成功		FALSE = 失敗
//-----------------------------------------------------------------------------------------
BOOL CDkRouter1000::PutFilterOA_LAN(char *tit)
{
...
//	<戻値>	TRUE = 成功		FALSE = 失敗
//-----------------------------------------------------------------------------------------
BOOL CDkRouter1000::PutFilter(char *tit)
{
	// 現在のルータの接続先最大設定数
...
//===== ↓クラウドIP電話対応 ==============================================================================
	...
	strPut.Format("ip filter set dk_pp_lan2 out 1 2 3 4 5 6 104 105 106 107 2015%s%s%s%s%s%s%s 2085 2086 2087 2088 2089%s%s 2097 2098 2099%s%s%s 10801 12650 12750",	// 12650 12750＝クラウドリニューアル時対応(サポ端末接続フィルタ追加)
...
//===== ↑クラウドIP電話対応 ============================================================
	...
	// ＊＊＊ ip filter set dk_lan3 in dynamic ＊＊＊
	...
	if ( m_sRouter.sLAN3.sConNaiyo[eL3XaiTact].fNeed )
	{
		if (!_Put(" 10655 10665 10675", FALSE))
			return FALSE;
	}
...
//	LAN/IP configuration
BOOL CDkRouter1000::PutLanIp(char *tit)
{
	char	szBuf[256];
...
BOOL CDkRouter1000::PutNatOther(char *tit)
{
	char	szBuf[256];
...
// 			return FALSE;
	...
	if ( m_sRouter.sLAN3.fVirusUse )
	{
		...
		if (!_Put(szBuf))
			return FALSE;
		wsprintf(szBuf, "nat descriptor static 1000 252 172.16.252.1=%s 1", GetIpB2C(m_sRouter.sLAN3.ipVirus));	// 変更　2019年機能追加(10.6 その他修正)
		if (!_Put(szBuf))
			return FALSE;
	}
	...
	if ( m_sRouter.sSIS05.no )  // SIS05通信先設定がある
	{
		...
		if ( m_sRouter.ncon[i].port == ePortL2 || m_sRouter.ncon[i].port == ePortL2B1 )	// L2接続
		{
			...
			wsprintf(szBuf, "nat descriptor static 2012 15 %s=172.16.15.100 1", GetIpB2C(bVPNadd));	// 変更　2019年機能追加(10.6 その他修正)
			if (!_Put(szBuf))	return FALSE;
		}
	...
	wsprintf(szBuf, "nat descriptor masquerade static 2000 5 %s udp 4500", GetIpB2C(m_sRouter.ip));	// 変更　2019年機能追加(10.6 その他修正)
	if (!_Put(szBuf))
		return FALSE;
...
//	NAT put(Ｃ店舗用)
BOOL CDkRouter1000::PutNat_C(char *tit)
{
	char	szBuf[256];
	...
	if ( m_sRouter.sLAN3.sConNaiyo[eL3XaiTact].fNeed )	// Χai-TACT接続に「○」を設定した場合のみ(書き込みはＣ店舗用の場合のみでよいとのこと)
	{
		BYTE ip[4] = {0};
...
//	NAT put(リモート店舗用)
BOOL CDkRouter1000::PutNat_R(char *tit)
{
	char	szBuf[256];
...
//	NAT put(本部店舗用)
BOOL CDkRouter1000::PutNat_H(char *tit)
{
	if (!_PutTit(tit))
		return FALSE;
...
//	PP の前処理
BOOL CDkRouter1000::PutPpHead(char *tit)
{
	if (!_PutTit(tit))
		return FALSE;
...
//===== ↑J-NET顧客代行対応 ==========================================================
		...
		if ( m_sRouter.ncon[n].type == eSIS05 )
		{
			if ( m_sRouter.ncon[n].port == ePortB1 || m_sRouter.ncon[n].port == ePortL2B1 )
			{
				...
				wsprintf(szBuf, "pp auth myname %s %s", m_sRouter.sSIS05.szHoleCode, GetIpB2C(m_sRouter.sSIS05.passIp));
				if (!_Put(szBuf))
					return FALSE;
			}
		...
		if ( m_sRouter.ncon[n].type == eADSL )
		{
			...
			// MTU値
			wsprintf(szBuf, "ip pp mtu %d", m_sRouter.nMTU);
			if (!_Put(szBuf))
				return FALSE;
...
//	PP anonymous
BOOL CDkRouter1000::PutPpAnonymous(char *tit)
{
...
{	
	...
	for ( int n = 0; n < SETU_MAX; n++ )
	{
		...
		//	title
		wsprintf(szBuf, "### IPSEC %d ###", nSetuzokuNo);
		if (!_PutTit(szBuf))			return FALSE;
	...
	if ( m_sRouter.fMent == eMentFixADSL || m_sRouter.fMent < 0 && (m_nMentType == eDKVPN || m_nMentType == eDKVPN_DialUp))
	{
		if ( !m_fMxRMAddRead )	// RMAddressを読み込んでいなければIPSEC自体書き込まない
		{
			return TRUE;
...
BOOL CDkRouter1000::PutOther(char *tit)
{
	char	szBuf[256];
...
// LAN3ポートが設定されているか?
BOOL CDkRouter1000::IsLAN3IpEnable(SDkLan3 *pRtLan3)
{
...
/*-----------------------------------------------------------------------------
    概要    ADSL情報の入った配列を取得
    引数    SDkRouter sRouter = ルータ設定
    戻値    SDkConnectInfoNew ADSLの接続先設定
-----------------------------------------------------------------------------*/
SDkConnectInfoNew *CDkRouter1000::GetAdslInfo(SDkRouter *pRouter/*=NULL*/)
{
	if ( pRouter == NULL )
	{
		pRouter = &m_sRouter;
	...
	for ( int i = 0; i < SETU_MAX; i++ ) {
		if ( pRouter->ncon[i].no == pRouter->m_nAdslNo ) { // 接続先設定構造体
			return &pRouter->ncon[i];
		}
	}
	return NULL;
}
...
//	<戻値>	TRUE = 成功    FALSE = 失敗
//----------------------------------------------------------------------------
BOOL CDkRouter1000::_PutFilterName(char *cmd)
{
	CString strGetLog;
	strGetLog = cmd;
...
//	<戻値>	TRUE = 成功    FALSE = 失敗
//----------------------------------------------------------------------------
BOOL CDkRouter1000::PutRevUpFile(BYTE *pSendIp, char *szPass/*=NULL*/)
{
...
//-----------------------------------------------------------------------------
int CDkRouter1000::PutTftp(BYTE *pSendIp, char *szSendFilePath, char *szPass/*=NULL*/)
{
	...
	char *pRouterPass = GetAdminPass(szPass);
	
	int no = 0;
	if ( !GetTftp(no, pSendIp, TRUE, pRouterPass) )	// いっぺん、ＧＥＴ処理をしてみる
		return 0;
	
	// tftp put
	wsprintf(szBuf, "cmd /c tftp %s put %s config/%s >%s", GetIpB2C(pSendIp), szSendFilePath, pRouterPass, szReDir);
	_Run(szBuf);
	
	int nWait = (GetRtCategory(m_sRouter.kind) == eRtType1000 || GetRtCategory(m_sRouter.kind) == eRtType1000_OA) ? 30 : 3;
	
	for (int cnt = 0; cnt < (nWait * 10); cnt++, Sleep(100))	// 60~100秒Wait
		DoIdle(3);
	
	if ( !CheckResult(szReDir) )	// TMPファイルに作られたファイルを見て、成功か失敗かを判断
	{
		CString str;
		if ( isMentMod() )
		{
			str.Format("pass=%sでの設定送信に失敗しました", pRouterPass);
			DebDisp(str);
		}
		...
		// デフォルトパスで送信失敗したわけではないのならデフォルトパスで送信処理をしてみる
		wsprintf(szBuf, "cmd /c tftp %s put %s config/%s >%s", GetIpB2C(pSendIp), szSendFilePath, RT_PASS, szReDir);
		_Run(szBuf);
		...
		if ( !CheckResult(szReDir) ) {	// TMPファイルに作られたファイルを見て、成功か失敗かを判断
			if ( isMentMod() )
			{
				str.Format("pass=%sでの設定送信に失敗しました", RT_PASS);
				DebDisp(str);
			}
			return 0;
		}
	...
	if ( m_fSendBin )	// .binファイルも送る？
	{	
		if ( !PutRevUpFile(pSendIp, szPass) )
			return -1;
	}
...
//	<戻値>	TRUE = 成功    FALSE = 失敗
//----------------------------------------------------------------------------
BOOL CDkRouter1000::GetTftp(int &no, char *ip, BOOL fKindGet/*=FALSE*/, char *szPass/*=NULL*/)
{
	...
	// ファイル名作成
	wsprintf(m_szFileName, "%s\\%s\\%s", CDkMxBas::GetBaseDir(), ROUTER_TMP_DIR, RT_RECV_FILE);
	...
	DWORD dwAttri = GetFileAttributes(m_szFileName);	// 戻り値 0xFFFFFFFF はファイルなしの意味
	if ( dwAttri != 0xFFFFFFFF )
	{
		dwAttri &= 0xFFFFFFFE;
		SetFileAttributes(m_szFileName,dwAttri);	// 読み取り専用でなくする
	}
	
	//	TFTP run(TFTPで設定を取得してファイルに落とす)
	wsprintf(szBuf, "tftp -i %s get config/%s %s", ip, pRouterPass, m_szFileName);
	_Run(szBuf);
	
	DoIdle(5);
	
	// データを取得できたかどうかのチェック
	dwAttri = GetFileAttributes(m_szFileName);
	if ( dwAttri == 0xFFFFFFFF )	// TMPファイルに作られたファイルを見て、成功か失敗かを判断
	{
		if ( isMentMod() )
		{
	...
	if ( cStat.m_mtime > (cSysTime - sp) && cStat.m_mtime < (cSysTime + sp) )
	{
		if ( isMentMod() )
		{
			str.Format("pass=%sで受信成功しました", szPass);
			DebDisp(str);
		}
...
//	TFTPでルータから取得する
BOOL CDkRouter1000::GetTftp(int &no, BYTE *ip, BOOL fKindGet/*=FALSE*/, char *szPass/*=NULL*/)
{
	char szIp[32];
...
//	<戻値>	TRUE = 成功    FALSE = 失敗
//----------------------------------------------------------------------------
BOOL CDkRouter1000::ReadRegstryData()
{
	// リモートアドレス
...
//	<戻値>	TRUE = 成功    FALSE = 失敗
//----------------------------------------------------------------------------
BOOL CDkRouter1000::WriteRegstryData(BOOL fRMAddBrank/*=FALSE*/)
{
	// リモートアドレス
