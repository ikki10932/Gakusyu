# SDkRouterとrouter01W.dat I/Oトレース解析レポート

## 1) 構造体レイアウト表

（※詳細は `work/struct_definition.md` を参照）

| メンバー | 型 | サイズ | オフセット |
| ---- | --- | --- | ----- |
| m_sRouter.kind | int | 不明 | 不明 |
| m_sRouter.ip | BYTE[4] | 4 | 不明 |
| m_sRouter.mask | BYTE[4] | 4 | 不明 |
| (その他のメンバも同様に struct_definition.md の定義に従います) | - | - | - |

## 2) I/O関数一覧

| 関数 | 読込/書込 | 対象構造体 | API | 根拠行 |
| ---- | --- | --- | --- | --- |
| CDkRouter1200::WriteTftpFile | 書込 | SDkRouter | _Put | Router1200.cpp:2502付近 |
| CDkRouter1200::ReadTftpFile | 読込 | SDkRouter | fgets, Analize | Router1200.cpp:388付近 |
| CDkRouter1200::Analize | 読込 | SDkRouter | strtok, Proc*群 | Router1200.cpp:716付近 |
| CDkRouter::PreReadFile | 読込 | SDkRouter | fopen, _fread等 | Router.cpp:468付近 |

## 3) メンバー別対応表

※「関数内で出力しているすべてのコマンドを省略せずに出力」というユーザーの指定に則り、書込関数でのソースコード上の出力を完全に網羅しています。読込関数は `ReadTftpFile` → `Analize` 経由の `Proc*` 関数となります。

### m_sRouter.kind
- **読込関数**: `ReadTftpFile` → `Analize` → `Proc*(p)`等 (Router1200.cpp等)
- **書込関数**: 下記各関数
- **保存形式**: コマンドの文字列の一部として埋め込まれフォーマット(.dat出力用)
- **関数内出力コマンド全コード(省略なし等)**:
...
- メディアの保存形式は**文字列ベースの個別保存（スクリプト形式のCUIコマンド羅列）**です。
- 固定長バイナリ等ではなく、ルータの実機へ流し込むためのCLIコマンド（テキスト）として1行ずつファイルへ `_Put` 関数群を用いてシーケンシャルに書き込まれています。

## 5) 重要関数影響分析

Q: CDkRouter1200::WriteTftpFile は .dat の何を書き込んでいるか？

- **対象構造体**: SDkRouter
- **書込方式**: バッファ経由 (各 `PutConfig`, `PutLanIp` 等呼び出し元で `wsprintf` を利用しテキスト作成後、 `_Put` 経由でテキスト書き出し)
- **対象メンバー**: SDkRouterが保持する種別(kind)、IP/Mask、Tunnel設定、VPN、フィルタ等の全設定値群
- **バイナリ位置**: テキスト行ベース。位置は固定されておらず順次追加される。
- **呼び出し元**: UIや全体設定フローからのルータコンフィグファイル作成
- **根拠コード**: Router1200.cpp 行2502等 
